"""
Benchmark script for running FILO2 solver on multiple instances.

This script:
- Runs 10-20 instances in parallel using FILO2
...
SRC_ROOT = ROOT.parent  # src (where master module is)

# Add src to path so Python can find master module
...
def solve_instance(instance_name: str, output_dir: Path, no_improvement: int = 100000) -> dict:
    ...
    try:
        ...
        # Solve the instance - pass output_dir so FILO writes directly there
        # (we'll still write the standardized solution ourselves)
        result = solve(
            ...
            solver_options={
                ...
                "seed": 1,
                "solution_dir": output_dir,  # FILO will write here, then we write standardized version
            }
        )
        ...
        _write_solution(
            ...
            solver="FILO2",  # Override solver name for .sol file
            runtime=result.runtime,
            stopping_criteria=stopping_criteria,
            gap_percent=gap,
        )
        
        # Clean up FILO2's native output files (keep only our standardized format)
        instance_stem = Path(instance_name).stem
        instance_name_with_ext = Path(instance_name).name
        filo_output_patterns = [
            output_dir / f"{instance_name_with_ext}_seed-{result.metadata.get('seed', 1)}.vrp.sol",
            output_dir / f"{instance_stem}_seed-{result.metadata.get('seed', 1)}.vrp.sol",
            output_dir / f"{instance_name_with_ext}_seed-{result.metadata.get('seed', 1)}.out",
            output_dir / f"{instance_stem}_seed-{result.metadata.get('seed', 1)}.out",
        ]
        for filo_file in filo_output_patterns:
            if filo_file.exists() and filo_file != output_dir / f"{instance_stem}.sol":
                try:
                    filo_file.unlink()
                except Exception:
...
def main():
    parser = argparse.ArgumentParser(
        description="Run FILO2 benchmark on multiple instances",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python benchmark_filo2.py output/solutions
  python benchmark_filo2.py output/solutions --no_improvement 5000
  python benchmark_filo2.py output/solutions --max_workers 4
        """
    )
    parser.add_argument(
        "output_path",
