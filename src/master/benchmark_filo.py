"""
Benchmark script for running FILO solver on multiple instances.

This script:
- Runs 10-20 instances in parallel using FILO
...
def solve_instance(instance_name: str, output_dir: Path, no_improvement: int = 100000) -> dict:
    ...
    try:
        ...
        _write_solution(
            ...
            solver="FILO",  # Override solver name for .sol file
            runtime=result.runtime,
            stopping_criteria=stopping_criteria,
            gap_percent=gap,
        )
        
        # Clean up FILO's native output files (keep only our standardized format)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run FILO benchmark on multiple instances",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python benchmark_filo.py output/solutions
  python benchmark_filo.py output/solutions --no_improvement 5000
  python benchmark_filo.py output/solutions --max_workers 4
        """
    )
    parser.add_argument(
        "output_path",
