#!/usr/bin/env python
"""
Box plots of gap_to_bks_percent (y) by depot position: R, E, C
(from instances_characteristics.json field depot_position).
"""
...
DEFAULT_OUT = ROOT / "results" / "out" / "gap_vs_depot_position.png"

BUCKET_ORDER = ("R", "E", "C")


def depot_position_bucket(raw: str) -> str:
    s = raw.strip()
    if s in BUCKET_ORDER:
        return s
    raise ValueError(f"Unmapped depot_position: {raw!r} (expected one of {BUCKET_ORDER})")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_OUT,
        help="Output image path (default: results/out/gap_vs_depot_position.png)",
    )
    ...
    with SUMMARY_PATH.open(encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            raw = str(chars[key]["depot_position"])
            b = depot_position_bucket(raw)
    ...
    ax.set_xlabel("Depot Position")
    ax.set_ylabel("Gap to BKS (%)")
    ax.set_title("Gap to best known solution vs depot position")
