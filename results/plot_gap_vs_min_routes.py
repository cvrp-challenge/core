#!/usr/bin/env python
"""
Scatter plot: minimum routes k (from instances_characteristics.json) vs
...
DEFAULT_OUT = ROOT / "results" / "out" / "gap_vs_min_routes.png"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_OUT,
        help="Output image path (default: results/out/gap_vs_min_routes.png)",
    )
    ...
    with SUMMARY_PATH.open(encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            min_routes.append(int(chars[key]["min_routes"]))
    ...
    x = np.asarray(min_routes, dtype=np.float64)
    ...
    ax.set_xlabel("Min. Routes (k)")
    ax.set_ylabel("Gap to BKS (%)")
    ax.set_title("Gap to best known solution vs minimum routes")
