#!/usr/bin/env python
"""
Scatter plot: vehicle capacity Q (from instances_characteristics.json) vs
...
DEFAULT_OUT = ROOT / "results" / "out" / "gap_vs_capacity.png"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_OUT,
        help="Output image path (default: results/out/gap_vs_capacity.png)",
    )
    ...
    with SUMMARY_PATH.open(encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            capacities.append(float(chars[key]["capacity"]))
    ...
    ax.set_xlabel("Capacity (Q)")
    ax.set_ylabel("Gap to BKS (%)")
    ax.set_title("Gap to best known solution vs capacity")
