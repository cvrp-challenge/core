#!/usr/bin/env python
"""
Box plots of gap_to_bks_percent (y) by coarse customer distribution bucket:
R, C, RC — derived from instances_characteristics.json (suffixes like "C (6)"
and "RC (2)" are collapsed to C and RC).
"""
...
DEFAULT_OUT = ROOT / "results" / "out" / "gap_vs_customer_distribution.png"

BUCKET_ORDER = ("R", "C", "RC")


def customer_distribution_bucket(raw: str) -> str:
    s = raw.strip()
    if s.startswith("RC"):
        return "RC"
    if s.startswith("C"):
    ...
    if s == "R":
        return "R"
    raise ValueError(f"Unmapped customer_distribution: {raw!r}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_OUT,
        help="Output image path (default: results/out/gap_vs_customer_distribution.png)",
    )
    ...
    with SUMMARY_PATH.open(encoding="utf-8", newline="") as f:
        ...
        for row in reader:
            ...
            raw = str(chars[key]["customer_distribution"])
            b = customer_distribution_bucket(raw)
    ...
    ax.set_xlabel("Customer Distribution")
    ax.set_ylabel("Gap to BKS (%)")
    ax.set_title("Gap to best known solution vs customer distribution")
