#!/usr/bin/env python
"""
Plot per-instance improvement-source split vs average route length.

Y-axis: percentage (0-100)
X-axis: average route length (n/k)

Lines:
- % of improvements from routing
- % of improvements from scp

Plus linear trend lines for each series.
"""
...
DEFAULT_OUT = ROOT / "results" / "out" / "improvement_split_vs_avg_route_length.png"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=DEFAULT_OUT,
        help="Output image path (default: results/out/improvement_split_vs_avg_route_length.png)",
    )
    ...
    ax.set_xlabel("Avg. Route Length")
    ax.set_ylabel("Improvement Share (%)")
    ax.set_ylim(0.0, 100.0)
    ax.set_title("Improvement-source split vs average route length")
